...
export function LoginPage() {
  ...
  useEffect(() => {
    if (session && profile) {
      ...
      navigate(requested || (profile.rol === 'admin' ? '/' : profile.rol === 'cliente' ? '/cliente' : profile.rol === 'fichas' ? '/fichas' : profile.rol === 'importaciones_lara' ? '/importaciones-lara' : '/arquitectura/resumen'), { replace: true });
    }
  }, [session, profile, location.state, navigate]);

  if (session && profile) return <Navigate to={profile.rol === 'admin' ? '/' : profile.rol === 'cliente' ? '/cliente' : profile.rol === 'fichas' ? '/fichas' : profile.rol === 'importaciones_lara' ? '/importaciones-lara' : '/arquitectura/resumen'} replace />;
